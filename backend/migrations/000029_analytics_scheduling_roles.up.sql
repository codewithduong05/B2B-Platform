...
-- Enable pg_cron extension and schedule jobs if available
DO $body$
BEGIN
    CREATE EXTENSION IF NOT EXISTS pg_cron;
    PERFORM cron.schedule('daily-order-rollup', '0 2 * * *', $$
        SELECT analytics.run_daily_order_rollup(CURRENT_DATE - 1);
    $$);
    PERFORM cron.schedule('daily-payment-rollup', '0 2 * * *', $$
        SELECT analytics.run_daily_payment_rollup(CURRENT_DATE - 1);
    $$);
    PERFORM cron.schedule('daily-inventory-rollup', '0 2 * * *', $$
        SELECT analytics.run_daily_inventory_rollup(CURRENT_DATE - 1);
    $$);
    PERFORM cron.schedule('daily-promotion-rollup', '0 2 * * *', $$
        SELECT analytics.run_daily_promotion_rollup(CURRENT_DATE - 1);
    $$);
    PERFORM cron.schedule('daily-financial-rollup', '0 2 * * *', $$
        SELECT analytics.run_daily_financial_rollup(CURRENT_DATE - 1);
    $$);
    PERFORM cron.schedule('daily-erp-sync-rollup', '0 2 * * *', $$
